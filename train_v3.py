...
def sample_images(batches_done,val_dataloader,generator,save_type):
    ...
    save_image(img_sample,'images_final/%s/%s.png'%(save_type,batches_done),normalize=True)
...
def test_image(batch_done, dataloader_val, generator, save):
    ...
    save_image(image_grid, 'images_final/%s/%d.png' % (save, batch_done), normalize=True)


def main(config):
    cudnn.benchmark = True
    os.makedirs('saved_models_final/%s/' % (config.loss_type), exist_ok=True)
    os.makedirs('images_final/%s/'%(config.loss_type), exist_ok=True)
    ...
    discriminator = MultiScaleDiscriminator(1)
    ...
    if config.loss_type=='WGAN-GP':
        criterion_gan=loss_v2.WassersteinLoss(discriminator,stain_generator)

    elif config.loss_type=='BCE-loss':
        criterion_gan=loss_v2.StandardGAN(discriminator,stain_generator)

    elif config.loss_type=='LS-loss':
        criterion_gan=loss_v2.LSGAN(discriminator,stain_generator)

    elif config.loss_type=='Logistic-loss':
        criterion_gan=loss_v2.LogisticGAN(discriminator,stain_generator)
    ...
    if config.epoch != 0:
        stain_generator.load_state_dict(
            torch.load('saved_models/%s/generator_unstained%d.pth' % (config.save_checkpoint,config.epoch)))

        # discriminator.load_state_dict(
        #     torch.load('saved_models/%s/discriminator_unstained%d.pth' % (config.save_checkpoint,config.epoch)))
        discriminator.load_state_dict(
            torch.load('saved_models/%s/discriminator_stained%d.pth' % (config.save_checkpoint,config.epoch)))
    ...
    for epoch in range(config.epoch, config.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            loss_dis, loss_gan, loss_fm = criterion_gan.loss(real_unstained, real_stained, 1)
            ...
            loss_dis.backward()
            optimizer_D.step()
                # loss_D = loss_dis + loss_dis_pixel
            batch_done = epoch * len(dataloader) + i
            print(
                '[epoch: %d/%d] [batch: %d/%d] [D loss: %f fm loss %f] [generator loss: %f ] [finished: %f percent]'
                % (
                    epoch,
                    config.n_epochs,
                    i,
        ...
        if (epoch+1) % config.checkpoints_interval == 0:
            torch.save(stain_generator.state_dict(),
                       'saved_models_final/%s/generator_stained%d.pth' % (config.loss_type,epoch))
            # torch.save(discriminator.state_dict(),
            #            'saved_models/%s/discriminator%d.pth' % (config.loss_type,epoch))
            torch.save(discriminator.state_dict(),
                       'saved_models_final/%s/discriminator_unstained%d.pth' % (config.loss_type,epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ngf', type=int, default=64, help='num channel filtes')
