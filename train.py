...
def sample_images(save_type,batches_done,val_dataloader,generator):
    '''
    '''
...
def main(config):
    ...
    os.makedirs('saved_models/%s/' % (config.loss_type), exist_ok=True)
    os.makedirs('images/%s/'%(config.loss_type), exist_ok=True)
    ...
    stain_generator = GeneratorUNet(1)
    ...
    for epoch in range(config.epoch, config.n_epochs):
        ...
        if (epoch+1) % config.checkpoints_interval == 0:
            torch.save(stain_generator.state_dict(),
                       'saved_models/%s/generator_stained%d.pth' % (config.loss_type,epoch))
            torch.save(pixel_discriminator.state_dict(),
                       'saved_models/%s/discriminator_unstained%d.pth' % (config.loss_type,epoch))
